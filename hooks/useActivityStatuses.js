  ...
  React.useEffect(() => {
    ...
    const fetchStatuses = async () => {
      try {
        ...
        if (!response.ok) {
          throw new Error("Failed to load activity statuses");
        }
        ...
        if (isMounted) {
          ...
          setStatuses([]);
          setError(err.message || "Failed to load activity statuses");
        }
      } finally {
        if (isMounted) {
          setIsLoading(false);
...
export default useActivityStatuses;
