...
const validationSchema = Yup.object().shape({
  ...
  sequencePadding: Yup.number().typeError("Must be a number").integer("Must be an integer").min(1, "Min 1").max(12, "Max 12"),
  nextSequenceNumber: Yup.number().typeError("Must be a number").integer("Must be an integer").min(1, "Min 1"),
  isActive: Yup.boolean(),
});

export default function EditCategoryModal({ fetchItems, item, currentPage = 1, currentSearch = "", currentPageSize = 10 }) {
  ...
  const handleSubmit = async (values, { setSubmitting }) => {
    try {
      ...
      const response = await fetch(`${BASE_URL}/HelpDesk/UpdateCategory`, {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({
          ...
          ticketPrefix: values.ticketPrefix || undefined,
          ticketSuffix: values.ticketSuffix || undefined,
          sequencePadding: values.sequencePadding || undefined,
          nextSequenceNumber: values.nextSequenceNumber || undefined,
        }),
      });
        ...
        <Box sx={style}>
          ...
          <Formik
            initialValues={{
              ...
              ticketPrefix: item.ticketPrefix || "HD",
              ticketSuffix: item.ticketSuffix || "",
              sequencePadding: item.sequencePadding ?? 6,
              nextSequenceNumber: item.nextSequenceNumber ?? 1,
                ...
                <Grid container spacing={2}>
                  ...
                  <Grid item xs={12} sm={6}>
                    <Field
                      ...
                      error={touched.nextSequenceNumber && !!errors.nextSequenceNumber}
                      helperText={touched.nextSequenceNumber && errors.nextSequenceNumber}
                    />
                  </Grid>
